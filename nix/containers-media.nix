...
let
  ...
  quadlet =
    ...
    ''
      ...
      ${lineSection "PublishPort" ports}${lineSection "Volume" volumes}${lineSection "Environment" environment}${lib.optionalString (user != null) "User=${user}\n"}${lib.optionalString (group != null) "Group=${group}\n"}
      [Service]
      Restart=always
      RestartSec=10s
      ...
      { config, pkgs, ... }:
      {
        ...
        home.activation.enableMediaQuadlets = config.lib.dag.entryAfter [ "writeBoundary" ] ''
          if command -v systemctl >/dev/null 2>&1 && systemctl --user is-system-running >/dev/null 2>&1; then
            ...
            systemctl --user enable --now ${lib.concatMapStringsSep " " (name: "${name}.service") mediaServices} || true
          fi
        '';
      }
    )
