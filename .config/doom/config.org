#+title: Evan's Literate Doom Emacs Configuration
#+author: Evan Riley
#+property: header-args :tangle config.el
#+startup: fold

This is the source of truth for my Emacs setup on =cinderace=.  The generated
=config.el=, Doom's =init.el=, and =packages.el= remain tracked so the setup can
bootstrap cleanly, but changes should start here.
...
** Systemd Service
Tracked as =~/.config/systemd/user/emacs.service=:
#+begin_example
[Unit]
Description=Emacs text editor daemon
Documentation=info:emacs man:emacs(1) https://gnu.org/software/emacs/
ConditionPathExists=%h/.emacs.d/init.el
ConditionPathExists=%h/.config/doom/init.el
ConditionFileIsExecutable=/usr/bin/emacs
...
Run =doom-bootstrap= after installing Emacs. It installs Doom, synchronizes the
configuration, and restarts the enabled user service.

** Desktop Entry
Tracked as =~/.local/share/applications/emacsclient.desktop=:
#+begin_example
[Desktop Entry]
Name=Emacs Client
...
Exec=/var/home/evan/bin/e --wait %F
Icon=emacs
Type=Application
Terminal=false
